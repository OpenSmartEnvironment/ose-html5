...
exports.key = function(that, ev) {  // {{{2
  ...
  switch (ev.code || ev.keyIdentifier) {
  case 'ArrowLeft':
    val = -1;
    // NO BREAK
  case 'ArrowRight':
...
exports.params = function(that, val) {  // {{{2
  ...
  if (val.aimWrap) {
    val.aimWrap.on('patch', patchAim.bind(that));
  }
  delete val.aimWrap;
...
// Private {{{1
function patchAim(patch, val) {  // {{{2
  console.log('PATCHING AIM', patch);
}
