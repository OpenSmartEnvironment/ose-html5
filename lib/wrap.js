...
exports.view2 = function(el, stateObj) {  // {{{2
/** TODO: Rename to `view`
 *
 * Create new view
 *
 ...
 */
  ...
  var res;
  if (stateObj.view.charAt(0) === '.') {
    res = this.O.new(stateObj.view)(el);
  } else {
    if (stateObj.view.indexOf('/') < 0) {
      res = O.new('./view/' + stateObj.view)(el);
    } else {
      res = this.O.new(stateObj.view)(el);
    }
  }

  res.stateObj = stateObj;
...
exports.button = function(action, text, cb) {  // {{{2
  this.append('<a>', 'button char')
...
//    console.log('SLIDER CLICK', val, ev.clientX, el.left(), el.width());
  ...
  el.append('div').text(' ').on('mousedown', function(ev) {
    el.stop(ev);
...
function buttonsVal(that, args) {  // {{{2
  ...
  case 0:
    ...
    var res = {};
    that.el.querySelectorAll('a').forEach(function(btn) {
      res[btn.action] = btn.getAttribute('value');
    });
    return res;
  case 1:
    args = args[0];
    ...
    case 'single':
      if (that.el.value) {
        that.el.querySelector('a[action="' + that.el.value + '"]').removeAttribute('value');
      }
      that.el.value = args;
      that.el.querySelector('a[action="' + that.el.value + '"]').setAttribute('value', 1);
      return;
    }

    return that.el.querySelector('a[action="' + args + '"]').value;
  case 2:
    O.log.todo();
    return;
  }
...
function transitProgress(el, aim, max, duration) {  // {{{2
  // <html5-progress> has some bug, setting to value zero and then setting some filltime doesn't work
