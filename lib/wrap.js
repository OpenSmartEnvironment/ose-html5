...
var FieldWrap = O.class('ose/lib/orm/wrap');
 ...
 */
  ...
  if (! this.control) {
    this.control = exports.DefaultControl;
  }
...
/**
 * Remove hook between an element and its wrapper
 *
...
/** TODO: Rename to `view`, consider to append view to the current element automaticaly
 *
 * Create new view
 *
 ...
 */
  ...
  /*
  el.dispatchEvent(new CustomEvent('removed', {
    bubbles: false,
    cancelable: false,
    wrap: this,
  }));
...
exports.off = function(name, cb, useCapture) {  // {{{2
/**
...
exports.setField = function(field) {  // {{{2
  this.el.ose = this;
  this.field = field;

  if (this.control.fieldUpdated) {
    field.onUpdate(this.control.fieldUpdated.bind(null, this));
    return this;
  }

  field.onUpdate(Function('field', 'patch', 'this.val(field.field.asEditText(field.value));').bind(this, field));
  return this;
};

exports.controls = {};  // HTML Controls {{{1

exports.DefaultControl = {  // {{{2
};
...
exports.register = function(name, control) {  // {{{2
  ...
  if (control.extend) {
    C.prototype[name] = control.extend;
  }
};

// Private {{{1
function listenRemoved(el, cb) {
  if (! el.parentNode) {
    setTimeout(function() {
      if (! el.parentNode) {
...
function childRemoveObserver(el) {
...
/* OBSOLETE {{{1
function checkboxVal(that, args) {  // {{{2
  if (args && args.length) {
    args = args[0];
    ...
    that.el.setAttribute('value', Boolean(args));
    return that;
  }
...
function buttonsVal(that, args) {  // {{{2
  ...
  case 1:
    ...
    case 'single':
      if (that.el.value) {
        that.el.querySelector('i[act="' + that.el.value + '"]').removeAttribute('value');
      }
      that.el.value = args;
      ...
      that.el.querySelector('i[act="' + that.el.value + '"]').setAttribute('value', 1);
      return;
    }

    return that.el.querySelector('i[act="' + args + '"]').value;
  case 2:
...
function sliderVal(that, args) {  // {{{2
  switch (args.length) {
  case 0:
    return that.value;
  case 1:
...
exports.append = function(tag, attrs, params) {  // {{{2
/ **
 ...
 * /
  ...
  switch (typeof (tag || undefined)) {
  case 'string':
    var res = new C(tag, attrs, params);
...
exports.checkbox = function(params) {  // {{{2
  if (typeof params === 'function') {
    params = {cb: params};
  }

  var res = this.append('<span>', 'checkbox')
    .attr('focusable', true)
    .on('click', function(ev) {
      res.stop(ev);

      if (res.el.hasAttribute('readonly')) return;

      var val = ! checkboxVal(res);

      if (params.cb) params.cb(val);

      if (params.post && params.field) {
        params.field.owner.view.post(params.post, val);
      }
  ...
  if (
    params.readonly ||
    ! params.post && ! params.cb
  ) {
    res.el.setAttribute('readonly', undefined);
  }

  if (params.field) {
    res.setField(params.field);
  }
 ...
 * /
  ...
  case 'SPAN':
    if (this.hasClass('checkbox')) {
      return checkboxVal(this, arguments);
    }
  ...
  case 'H5':
    ...
    this.el.textContent = val.toString();
//    O.log.error(this, 'INVALID_ARGS', val);
    return this;
  /*
  case 'html5-CHECKBOX':
  case 'html5-SWITCH':
