...
/**
 * Display content based on `stateObj`
 *
...
//  console.log('CONTENT DISPLAY', stateObj, source, this.content && this.content.id);
  ...
  if (this.content) {
    if (this.content.verifyStateObj(stateObj, source)) {
      if (this.content.update) {
        return this.content.update(stateObj, cb);
      }
      return cb();
    }
...
//  this.search.hide(true);
  ...
  this.content = this.view2(stateObj);
...
/* OBSOLETE {{{1
exports.view = function(stateObj) {  // {{{2
/ *
 * Create view based on `stateObj`
 *
 * @param stateObj {Object} Object defining what is to be displayed
 *
 * @return {Object} View
 ...
 * /
  ...
  var result = this.newView(stateObj);
