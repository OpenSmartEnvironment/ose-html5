  ...
  */
  ...
  this.fields.each(function(fw) {
    if (fw.profile.order === 'header') {
      ...
      var fe = new Field(fw);
      div.append(fe);
      if (fe.getAsside) fw.li.add(fe.getAsside());
    }
...
function newEntry(that) {  // {{{2
  ...
  var so = that.stateObj;

  O.findShard(so.ident, function(err, shard) {
    if (err) return that.afterDisplay(err);

    shard.embryo(so.ident.id, so.ident.kind, so.dval, function(err, entry) {
      if (err) return that.afterDisplay(err);
