 ...
 */
  ...
  if (entry.kind.ddef) {
    this.dwrap = entry.kind.ddef.iterLayout(this.fields, this.so.layout || this.layout, entry.dval);
  }
  if (entry.kind.sdef) {
    this.swrap = entry.kind.sdef.iterLayout(this.fields, this.so.layout || this.layout, entry.sval);
  }
  this.fields.sort();
