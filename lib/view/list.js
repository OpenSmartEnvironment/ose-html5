...
//    console.log('LIST SHARDS', err, val);
  ...
  function setup(err, shard) {  // {{{3
    ...
    if (shard) {
      ...
      if (that.schema) {
        if (that.schema !== shard.schema) {
          return that.afterDisplay(O.error(that, 'INVALID_STATE_OBJ', 'Shard has different schema than specified one', that.so));
        }
      } else {
        that.schema = shard.schema;
      }
    ...
    if (ident.kind) {
      if (! that.schema) {
        return that.afterDisplay(O.error(that, 'INVALID_STATE_OBJ', 'Kind can\'t be found without schema defined', that.so));
      }

      that.kind = that.schema.kinds[ident.kind];
      if (! that.kind) {
        that.afterDisplay(O.error(that, 'INVALID_STATE_OBJ', 'Kind was not found within a schema', that.so));
      }
    }

    console.log('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX', that.kind && that.kind.name);
