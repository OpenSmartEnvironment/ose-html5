...
var O = require('ose').class(module, C, './detail');
...
// Public {{{1
function C() {  // {{{2
  O.super.call(this, 'section', 'gesture');
}
...
exports.verifyStateObj = function(val) {  // {{{2
  ...
  return O.inherited(this, 'verifyStateObj')(val);
};
 ...
 */
  ...
  this.append2('<br><span>' + message + '</span>');

  if (val) {
    this.tree('span').text(JSON.stringify(val));
  }
};
