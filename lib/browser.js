...
//  this.globalDrawer.addLink('New entry', 'newEntry', newEntry.bind(this));
  ...
  var stateObj = this.configData.defaultStateObj || history.state || {content: this.defaultStateObj};
 ...
 */
  ...
  console.log('NEW StateObj', {lastSource: this.lastSource, source: source}, JSON.stringify(stateObj));
...
// TODO    (! (this.globalDrawer.visible() || this.main.search.visible()))
  ...
  this.main.display(stateObj.content, source, cb);
...
/**
 * Creates new event handler that calls the "O.ui.display(stateObj)"
 *
 * @param stateObj {Object} State object to be displayed
 *
 * @returns {Function} Event handler calling "O.ui.display(stateObj)"
 *
 * @method bindContent
 */

  return function(ev) {
    O.ui.display({content: stateObj}, 'user', function(err) {
      if (err) O.log.error(err);
    });
...
//  console.log('GET STATE OBJ', this.rightBox.content);
  ...
  if (this.main.content) {
    result.content = this.main.content.stateObj;
  }
