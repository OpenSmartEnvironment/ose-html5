...
//  console.log('KEYDOWN', ev.code, ev.key);
  ...
  if (bubble(this.active, 'key', ev) === false) {
    stop(ev);
    return;
  }
  ...
  case 'Enter':
  case 'Space':
    if (bubble(this.active, 'tap', ev) === false) {
      stop(ev);
    }
    return;
  case 'ArrowUp':
...
//  console.log('MOUSE DOWN', ev, ev.target);
  ...
  if (this.active._on_drag || this.active.control && this.active.control.drag) {
    stop(ev);
  }
...
//  console.log('MOUSE MOVE', ev);
  ...
  case 'undefined':
    ...
    if (
      ma.wrap &&
      (ma.wrap._on_drag || ma.wrap.control && ma.wrap.control.drag) &&
      ma.wrap.trigger('drag', ma) === false
    ) {
      stop(ev);
      return;
    }
...
function testTap(ma) {  // {{{2
  ...
  if (bubble(ma.wrap, 'tap', ma) === false) {
    return true;
  }
}
