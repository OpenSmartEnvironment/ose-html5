...
function beginMouse(that, type, coord) {  // {{{2
  ...
  if (that.active) {
    ...
    var el = ma.target;
    while (el && el !== document.body) {
      if (el.ose && (el.ose._on_drag || el.ose.control && el.ose.control.drag)) {
        ma.draggable = el.ose;
        if (ma.tappable) break;
      }

      if (el.ose && (el.ose._on_tap || el.ose._on_hold || el.ose.control && (el.ose.control.tap || el.ose.control.hold))) {
        ma.tappable = el.ose;
